...
class Sim(VoxCadParams):
    ...
    def __init__(self, self_collisions_enabled=True, simulation_time=3, dt_frac=0.7, stop_condition=2,
...
class Env(VoxCadParams):
    ...
    def __init__(self, frequency=4.0, gravity_enabled=1, temp_enabled=1, floor_enabled=0, floor_slope=0.0,
                 lattice_dimension=0.01, fat_stiffness=1e+005, bone_stiffness=5e+008, muscle_stiffness=5e+006,
...
class BehavioralCharacterizationDict(dict):
    """A dictionary describing the behavioral characterization for optimization. See self.add_objective()."""

    def __init__(self):
        super(BehavioralCharacterizationDict, self).__init__()
        self.max_rank = 0

    def add_BC(self, name, tag, min, max, nb_cells, worst_value=-999,  logging_only=False):
        """Add an optimization behavioral characterization to the dictionary.
        ...
        tag : str or None
            ...
            If this is None then the attribute is calculated outside of VoxCad (in Python only).
        min: int or double
            The minumum value for this BC
        max: int or double
            The maximum value for this BC
        nb_cells: int
            number of cells for this BC for map discretization

        logging_only : bool
            If True then don't use as BC on map-elites, only to track statistics from the simulation.

        """
        curr_rank = self.max_rank

        super(BehavioralCharacterizationDict, self).__setitem__(curr_rank, {"name": name,
                                                           "tag": xml_format(tag) if tag is not None else None,
                                                           "min": min,
                                                           "max": max,
                                                           "nb_cells": nb_cells,
