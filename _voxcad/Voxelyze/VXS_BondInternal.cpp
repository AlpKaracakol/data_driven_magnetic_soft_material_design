...
void CVXS_BondInternal::CalcLinForce() //get bond forces given positions, angles, and stiffnesses...
{
	...
	Quat3D<double> CurXAng1(pVox1->GetCurAngleHighAccuracy());
	Quat3D<double> CurXAng2(pVox2->GetCurAngleHighAccuracy());
	...
	Quat3D<double> NewAng2(CurXAng1.Conjugate()*CurXAng2);
	...
	if (SmallAngle)	{ //Align so Angle1 is all zeros
		...
		Quat3D<double> Pos2AlignedRotAng;
