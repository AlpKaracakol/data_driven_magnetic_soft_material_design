...
CVX_Environment::~CVX_Environment(void)
{
	// ADDED FC: cleaning environmental sources
	for(int kk = 0; kk < environmentalSources.size(); kk++)
		delete environmentalSources[kk];
...
bool CVX_Environment::ReadXML(CXML_Rip* pXML, std::string* RetMessage) //pXML pointer to an "Environment" element
{
	...
	if (pXML->FindLoadElement("FloorRadius", &floorRadius)) 		
		...
		limitedFloor = false; // resets if not present

	// Added FC - let's load information regarding the sources from the .vxa file. They may be then accessed at voxel level in VXS_Voxel.cpp
	// CHECK XML POINTERS
	environmentalSources.clear(); // reset each time we load a new vxa.
	sourcesPresent = false; 
	
	if (pXML->FindElement("Sources"))
	{ 
		//std::cout<< "Tag Sources found" << std::endl; // DEBUG

		// Look for each source until one can be find
		for(int kk = 0; pXML->FindElement("Source"); kk++)
		{
			...
			double ampx,freqx,ampy,freqy,ampz,freqz; // source motion parameters

			// Look for all the features defining a source, otherwise initialize with default values
			if (!pXML->FindLoadElement("SourceName", &name)) name = std::string("NoNameSource");
			if (!pXML->FindLoadElement("SourceType", &type)) type = 0;
			if (!pXML->FindLoadElement("SourceEnabled", &enabled)) enabled = false;
			if (!pXML->FindLoadElement("SourcePositionX", &posx)) posx = 0;
			if (!pXML->FindLoadElement("SourcePositionY", &posy)) posy = 0;
			if (!pXML->FindLoadElement("SourcePositionZ", &posz)) posz = 0;

			if (!pXML->FindLoadElement("SourceMotionAmpX", &ampx)) ampx  = 0;
			if (!pXML->FindLoadElement("SourceMotionFreqX", &freqx)) freqx = 0;
			if (!pXML->FindLoadElement("SourceMotionAmpY", &ampy)) ampy  = 0;
			if (!pXML->FindLoadElement("SourceMotionFreqY", &freqy)) freqy = 0;
			if (!pXML->FindLoadElement("SourceMotionAmpZ", &ampz)) ampz  = 0;
			if (!pXML->FindLoadElement("SourceMotionFreqZ", &freqz)) freqz = 0;

			//std::cout << "name = " << name << std::endl 
			//		  << "Motion params = " << ampx << "," << freqx << ","<< ampy << "," << freqy << ","<< ampz << "," << freqz << std::endl;
			//		  << "type = " << type << std::endl
			//		  << "enabled = " << enabled << std::endl
			//		  << "x, y, z = " << posx << "," << posy << ","<< posz << std::endl; // DEBUG

			// Let's now create a new source and add it to the sources vector
			environmentalSources.push_back(new VX_Source(name, static_cast<SourceType>(type), enabled, posx, posy, posz, ampx, freqx, ampy, freqy, ampz, freqz));

			//std::cout << "Source added to environmental sources" << std::endl; // DEBUG
		}

		pXML->UpLevel(); // Check

		// DEBUG JUST FOR DEBUG, REMOVE
		//std::cout<< "Debug: quitting" << std::endl; // DEBUG
		//exit(1);
	}
