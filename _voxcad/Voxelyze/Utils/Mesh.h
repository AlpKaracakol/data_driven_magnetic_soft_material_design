...
struct CVertex {
	...
	Vec3D<> n; //normal
	Vec3D<> v; //Vertex location --> ***** MUST SUM DrawOffset TO GET THE POSITION OF THE VERTEX AFTER DEFORMATION *****
	CColor VColor;
...
struct CFacet {
	CFacet() {n=Vec3D<>(0,0,0); FColor = CColor(1,1,1,1); vi[0] = 0; vi[1] = 0; vi[2] = 1; Name = -1; drawPointingDirection = false;}
	CFacet(const int& av1, const int& av2, const int& av3) { n = Vec3D<>(0,0,0); vi[0] = av1; vi[1] = av2; vi[2] = av3; FColor = CColor(1,1,1,1); Name = -1;drawPointingDirection = false;}
	CFacet(const Vec3D<>& an, const int& av1, const int& av2, const int& av3) { n = an; vi[0] = av1; vi[1] = av2; vi[2] = av3; FColor = CColor(1,1,1,1); Name = -1;drawPointingDirection = false;}
	CFacet(const Vec3D<>& an, const int& av1, const int& av2, const int& av3, const int& NIn) { n = an; vi[0] = av1; vi[1] = av2; vi[2] = av3; FColor = CColor(1,1,1,1); Name = NIn;drawPointingDirection = false;}
	CFacet(const Vec3D<>& an, const int& av1, const int& av2, const int& av3, const CColor& ac) { n = an; vi[0] = av1; vi[1] = av2; vi[2] = av3; FColor = ac; Name = -1;drawPointingDirection = false;}
	...
	CFacet(const CFacet& p) {*this = p;drawPointingDirection = false;}

	Vec3D<> n; //normal vector
	CColor FColor;
	int vi[3]; //vertex indices (indices of the three vertex, use to index CVertex vector)
	int Name; //my name (for GL picking)
	bool drawPointingDirection;
};
