...
/*	// this solution here is not very elegant, dropped
	int count = 0;
	for(int i = 0; i < (int)Facets.size(); i++)
		if(Facets[i].drawPointingDirection)
		{
			...
			// the vector we need is either one of the two first (if there are only 2) or the last 2 (if there are 6)
			// -> could just save and plot the last

			CGL_Utils::DrawLineArrowD(VertPos, VertPos + Facets[i].n*0.1, 1.0, c);  
			count++;
		}*/
