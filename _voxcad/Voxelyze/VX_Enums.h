...
enum StopCondition {
	...
	SC_MIN_MAXMOVE_MAX_SIM_TIME, // !< combines SC_MIN_MAXMOVE and SC_MAX_SIM_TIME. Stop condition value is the max sim time, maxmove is hardcoded, based on voxel dim
	SC_POINTING_ERR // !< stop if abs(pointing error) < errorThreshold for thresholdTime
};
