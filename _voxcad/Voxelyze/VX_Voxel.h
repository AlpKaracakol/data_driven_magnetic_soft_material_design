...
public:
	...
	inline void SetExternalForce(const Vec3D<>& ForceIn) {ExternalForce = ForceIn;} //!< Convenience function for setExternalForce(float, float, float).
	...
	inline void SetExternalTorque(const Vec3D<>& TorqueIn) {ExternalTorque = TorqueIn;} //!< Convenience function for setExternalMoment(float, float, float).
