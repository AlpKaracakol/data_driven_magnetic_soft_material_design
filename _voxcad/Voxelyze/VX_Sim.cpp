...
double CVX_Sim::computePointingError(int step)
{
	...
	tipPosition = VoxArray[tipVoxel].GetCurPos();
	pointingVector = VoxArray[tipVoxel].GetCurAngle().RotateVec3D(initialPointingVector);
...
//#pragma omp parallel for
	...
	for (int i=0; i<iT; i++) {
		VoxArray[i].SetExternalTorque(Vec3D<>(0.,0.,0.));
		VoxArray[i].SetExternalForce(Vec3D<>(0.,0.,0.));

		VoxArray[i].SetCurMDirec(VoxArray[i].GetCurAngle().RotateVec3D(VoxArray[i].GetInitMDirec()));
		VoxArray[i].SetCurMoment(VoxArray[i].GetCurMMagnitude()*VoxArray[i].GetCurMDirec());

		if (isMagForceOn){
			/* calculate the force --> assumed to be 0 for now for a uniform magnetic field*/
			// measured B values for a permannet magnet Barr at the relative distances Zarr
			double Barr[] = {2.6e-3, 2.99e-3, 3.38e-3, 3.9e-3, 4.55e-3, 5.2e-3, 6.11e-3, 7.15e-3, 8.58e-3, 10.4e-3, 12.35e-3, 14.82e-3, 18.07e-3, 22.23e-3, 28.08e-3, 35e-3, 44.3e-3, 55.8e-3, 73.6e-3};
			double Zarr[] = {115e-3, 110e-3, 105e-3, 100e-3, 95e-3, 90e-3, 85e-3, 80e-3, 75e-3, 70e-3, 65e-3, 60e-3, 55e-3, 50e-3, 45e-3, 40e-3, 35e-3, 30e-3, 25e-3};
			double BGradarr[] = {-0.0780, -0.0780, -0.0910, -0.1170, -0.1300, -0.1560, -0.1950, -0.2470, -0.3250, -0.3770, -0.4420, -0.5720, -0.7410, -1.0010, -1.2770, -1.6220, -2.0770, -2.9300, -3.5660};

			// get the element number
			int numOfElements = sizeof(Barr)/sizeof(Barr[0]);
			//  get the current external B
			double curBext = VoxArray[i].GetCurBExt().z;

			// define the extForce due to magnetic gradient
			Vec3D<> extF = Vec3D<> (0,0,0);
			
			if (curBext>=Barr[0]) {

				int indexB = 0;
				// Find the index of the B ext
				for (int i = 0; i < numOfElements; i++) {
					if (curBext >= Barr[i]){
						indexB = i;
					}
				}
				// calculate the position of the voxel with respect to magnet
				double posZwrtMagnet = VoxArray[i].GetCurPos().z+Zarr[indexB];

				int indexZpos = 0;

				// calculate the Bgradient at the voxel position
				for (int i = 0; i < numOfElements; i++) {
					if (posZwrtMagnet >= Zarr[i]){
						indexZpos = i;
						break;
					}
				}
				...
				if (indexZpos > 0){
					Bgrad = posZwrtMagnet*(BGradarr[indexZpos]-BGradarr[indexZpos-1])/(Zarr[indexZpos]-Zarr[indexZpos-1]);
				}
				else {
					Bgrad = BGradarr[0];
				}
				// set the external B gradient
				VoxArray[i].SetCurBGradExt(Vec3D<>(0,0,Bgrad));

				extF = Vec3D<> (0,0,-VoxArray[i].GetCurMoment().Dot(VoxArray[i].GetCurBGradExt()));
			}
			else {
				// assuming the gradient is negligible 
				extF = Vec3D<> (0,0,0);

			}

			VoxArray[i].SetExternalForce(extF);
		}
		/* calculate the torque */
		/* apply the force and torque */
		VoxArray[i].SetExternalTorque(VoxArray[i].GetCurMoment().Cross(VoxArray[i].GetCurBExt()));
		if (VoxArray[i].IsExtDistF){
			VoxArray[i].AddExternalForce(VoxArray[i].GetFExtDist());
		}
...
Vec3D<> CVX_Sim::GetMaxMinPosZ(void)
{

	Vec3D<> MaxMinPosZ(9999,9999,9999);
	int nVox = NumVox();
	for (int i=0; i<nVox; i++){
		CVXS_Voxel* it = &VoxArray[i];
		if (it->GetCurPos().z < MaxMinPosZ.z)
		{
			MaxMinPosZ = it->GetCurPos();
		}
	}
...
int CVX_Sim::GetNumOnFloor()
{
	int NumTouching = 0;

	int LocNumVox = NumVox();
	vfloat Size = VoxArray[0].GetNominalSize();
	for (int i=0; i<LocNumVox; i++){
		Vec3D<double> Pos = VoxArray[i].GetCurPos();
		if (Pos.z - (1.5*Size) <= 0) NumTouching++;
	}
	return NumTouching;
}
